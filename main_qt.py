...
class ChatBubble(QtWidgets.QWidget):
    def __init__(self, message, role="assistant", think_content=None, last_json=None, parent=None, on_height_ready=None):
        ...
        # Move tooltip to icon_label for assistant
        if think_content and role == "assistant":
            ...
            def show_custom_tooltip(event):
                ...
                def actually_show():
                    # Only show if mouse is still over icon_label
                    if icon_label.underMouse():
                        # Offset tooltip to the right and below the mouse pointer
        ...
        # --- Add double-click event for assistant icon to show JSON ---
        if role == "assistant" and last_json is not None:
            def show_json_dialog(event):
                dlg = QtWidgets.QDialog(self)
                dlg.setWindowTitle("Ollama Request/Response JSON")
                dlg.resize(600, 400)
                ...
                dlg.exec()
            icon_label.mouseDoubleClickEvent = show_json_dialog
...
""")
    ...
    def add_chat_bubble(self, text, role="assistant", think_content=None, last_json=None):
        ...
        self.remove_thinking_bubble()
        bubble = ChatBubble(
            text, role=role, think_content=think_content, last_json=last_json,
    ...
    def update_chat(self, reply, think_content, last_json):
        self.remove_thinking_bubble()
        self.add_chat_bubble(reply, role="assistant", think_content=think_content, last_json=last_json)
    ...
    def ollama_query(self, text, last_json):
        ...
        def run():
            ...
            self.chat_history.append({
                ...
                "last_json": last_json  # <-- Add this line
            })
            if self.current_history_idx is not None:
                self.chat_histories[self.current_history_idx]["history"] = list(self.chat_history)
                self.refresh_chat_history_list()
